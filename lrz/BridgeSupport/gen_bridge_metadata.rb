...
class OCHeaderAnalyzer
  ...
  def self.path(path, fails_on_error=true, do_64=false)
    data = OCHeaderAnalyzer.do_cpp(path, fails_on_error, do_64)
    new(data, path)
  end
  ...
  def defines
    if @defines.nil?
      re = /#define\s+([^\s]+)\s+(\([^)]+\)|[^\s]+)\s*$/
      @defines = {}
      File.read(@path).scan(re).each do |m|
  ...
  def self.do_cpp(path, fails_on_error=true, do_64=false)
    f_on = false
    err_file = '/tmp/.cpp.err'
    result = `#{CPP} #{CPPFLAGS} #{do_64 ? '-D__LP64__' : ''} \"#{path}\" 2>#{err_file}`.select { |s|
...
class BridgeSupportGenerator
  ...
  def collect 
    ...
    if @generate_format == FORMAT_FINAL
      ...
      fix_method_types
      fix_struct_tags
    elsif @generate_format == FORMAT_DYLIB
      generate_precompiled_header
      collect_types_encoding
    end
  end
  ...
  def generate_xml_document
    ...
    when FORMAT_TEMPLATE
      @ocmethods.each do |class_name, methods|
        f = methods.select { |m| m.args.any? { |x| x.stripped_rettype == 'SEL' } }
        next if f.empty?
        element = root.add_element('class')
        element.add_attribute('name', class_name) 
        f.each do |method|
          celement = element.add_element('method')
          celement.add_attribute('selector', method.selector)
          celement.add_attribute('class_method', true) if method.class_method?
          method.args.each_with_index do |arg, i|
            next unless arg.stripped_rettype == 'SEL'
            aelement = celement.add_element('arg')
            aelement.add_attribute('index', i)
            aelement.add_attribute('sel_of_type', '???')
          end
        end 
     end
=begin
...
=end
    when FORMAT_FINAL
      ...
      @resolved_structs.sort { |x, y| x[0] <=> y[0] }.each do |name, type|
      ...
      @resolved_enums.to_a.sort { |x, y| x[0] <=> y[0] }.each do |enum, value| 
      ...
      @ocmethods.sort { |x, y| x[0] <=> y[0] }.each do |class_name, methods|
        elements = methods.sort.map { |method| 
          ...
          next if !custom_retval and custom_args.empty?
          element = REXML::Element.new('method')
          element.add_attribute('selector', method.selector)
          element.add_attribute('class_method', true) if method.class_method?
  ...
  def merge_document_with_exceptions(document, exception_document)
     ...
     func_name = func_element.attributes['name']
      ...
      func_element.elements.each('arg') do |arg_element|
        ...
        arg_element.attributes.each do |name, value|
          ...
          if name == 'type'
            ...
            if value.nil?
              errors << "Encoding of function argument '#{func_name}' type '#{orig_type}' not resolved" 
              next
            end
          end
      ...
      if retval_element
        ...
        if type_name
          ...
          if type.nil?
            errors << "encoding of function return type '#{func_name}' not resolved" 
          else
      ...
      class_element.elements.each('method') do |element|
        ...
        if retval_element
          ...
          if type_name
            type = @types_encoding[type_name]
            if type.nil?
              errors << "encoding of method return type '#{class_element.attributes['selector']}' not resolved"
            else
        ...
        element.elements.each('arg') do |arg_element|
          ...
          if type_name
            type = @types_encoding[type_name]
            if type.nil?
              errors << "encoding of method arg type '#{class_element.attributes['selector']}' not resolved" 
            else
              arg_element.add_attribute('type', type)
            end
          end
          sel_type = arg_element.attributes['sel_of_type']
          if sel_type
            sel = @sel_types[sel_type].selector
            types = @resolved_inf_protocols_encoding[sel].to_a.uniq
            if types.empty?
              errors << "selector type of method arg '#{sel_type}' not resolved"
            else
              arg_element.add_attribute('sel_of_type', types[0])
              arg_element.add_attribute('sel_of_type64', types[1]) if types.size > 1
            end
          end
        end
      end
    ...
    unless errors.empty?
      $stderr.puts "Fatal errors when merging the exceptions:\n#{errors.join("\n")}"
      exit 1
    end
  end
  ...
  def scan_headers
    ...
    @headers.each do |path|
      ...
      analyzer = OCHeaderAnalyzer.path(path, !@private, @enable_64)
      case @generate_format
      when FORMAT_DYLIB
  ...
  def handle_framework(val)
    ...
    raise "Can't locate framework library at '#{libpath}'" unless File.exist?(libpath)
    OBJC.dlload(libpath)
  ...
  def delete_precompiled_header
    ...
    @import_directive = @import_directive_before_pch
    @compiler_flags = @compiler_flags_before_pch
    @pch_files = nil
  end
end
...
def measure(task)
  started = Time.now
  yield
  $stderr.puts "finished #{task} (#{Time.now - started} seconds)" if $DEBUG
end
...
if __FILE__ == $0
  ...
  OptionParser.new do |opts|
    ...
    if ARGV.empty?
      ...
      begin
        opts.parse!(ARGV)
        g.headers.concat(ARGV)
        measure('32-bit collect') { g.collect }
        if enable_64
          g2 = g.duplicate
          g2.enable_64 = true
          ...
          measure('64-bit collect') { g2.collect }
          measure('32-bit and 64-bit metadata merge') { g.merge_64_metadata(g2) }
        end
        measure('metadata gen/write') { g.write }
        g.cleanup
      rescue => e
        msg = e.message
